...
🔧 Tools — "model-controlled". Модель сама вирішує, коли і з якими аргументами їх викликати — без додаткового коду з боку розробника. Кожен tool описується трьома речами: назва, description (природня мова — саме по ній модель розуміє навіщо цей tool), і JSON Schema параметрів. Важливий нюанс: якщо description нечітке або занадто загальне — модель може викликати tool не до місця або взагалі його ігнорувати. Якість description прямо впливає на якість поведінки агента. Tools можуть мати side effects: записати файл, відкрити PR, надіслати повідомлення — тому саме вони вимагають найбільшої уваги з точки зору безпеки.

📄 Resources — "app-controlled". Це read-only дані, доступні за URI-схемою: `file:///project/README.md`, `db://users/42`, `git://repo/main/src/index.ts`. На відміну від tools, їх показує хост-додаток, а не модель. Тобто розробник або адміністратор системи вирішує, які ресурси взагалі видимі. Поточний стан: вкрай недовикористані — більшість команд навіть не знають, що ця можливість є. А потенціал величезний: онбординг-документація, architecture decision records, design tokens, специфікації API — все це можна зробити resources і вони будуть доступні агенту без необхідності їх вручну копіювати в контекст.

💬 Prompts — "user-controlled". Це готові шаблони, які користувач може запустити вручну — через меню, slash-команду, чи кнопку. Приклади: `/review-pr`, `/explain-error`, `/generate-test-for-this-component`. Вони не виконуються автоматично — тільки за явним запитом. Корисні для стандартизації: якщо команда завжди хоче робити code review за одним і тим самим checklist-ом — це промпт у MCP-сервері, а не документ у Notion який всі забувають відкрити. Теж недовикористані, але для аутсорс-команд — реальна можливість упакувати свої best practices в інструмент.
-->
