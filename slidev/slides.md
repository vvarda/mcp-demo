...
🖥 STDIO — уяви що ти відкриваєш програму і вона одразу починає "розмовляти" з іншою програмою через термінал: один пише в stdin, інший читає з stdout. Ніяких портів, ніяких налаштувань мережі. Саме тому STDIO — це перше що пробують: запустив один рядок в терміналі і сервер вже підключений. Майже всі локальні MCP-сервери (Filesystem, GitHub, Playwright) працюють саме так. Мінус: тільки на одній машині, не можна зробити хмарний сервіс.

🌐 Streamable HTTP — це коли сервер живе десь в інтернеті, а не на твоєму ноутбуці. Запити йдуть як звичайні HTTP POST, відповіді можуть стримитись назад у реальному часі. Цей транспорт у 2026 повністю замінив старий формат (HTTP+SSE) — тепер є стандартний заголовок `Mcp-Session-Id` щоб відстежувати сесію. Корисно коли ти хочеш дати доступ до MCP-сервера цілій команді, а не тільки собі локально.

⚙️ Custom — це для складних enterprise-сценаріїв: gRPC для низької затримки, WebSocket для постійного з'єднання, щось своє для IoT або edge-пристроїв. На практиці 99% команд ніколи не торкаються цього варіанту — але важливо знати що протокол не прив'язаний до конкретного транспорту і за потреби можна зробити будь-що.
-->
