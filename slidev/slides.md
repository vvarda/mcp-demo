...
Простіше: уяви що підключення MCP-сервера — це як дати AI нову "інструкцію з роботи". Він її прочитав — і тепер знає що може зробити. Без підключення — не знає що є такий інструмент взагалі.
...
Приклад із Sentry: сервер виставляє `search_issues` і `get_issue_details`. Ти пишеш "знайди root cause" — модель сама розуміє що треба викликати `get_issue_details`, а не `search_issues`. Вона вибирає по description, не по назві функції. Саме тому хороший опис tool'а — це половина успіху.
-->
